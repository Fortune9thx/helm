...
const STATUS_COPY: Partial<Record<TransactionStatus, string>> = {
  ...
  [TransactionStatus.PROPOSING]: "Leader evaluating policy against live data…",
          ...
          {VALIDATOR_INDICES.map((vIdx) => {
            ...
            return (
              <motion.line
                ...
                x2={to.x}
                y2={to.y}
                stroke={failed ? "rgba(255,59,92,0.5)" : "rgba(0,240,255,0.55)"}
        ...
        <AnimatePresence mode="wait">
          <motion.p
            ...
            className={cn(
              ...
              failed ? "text-rose" : consensusReached ? "text-cyan" : "text-text-secondary"
            )}
          >
...
function ConsensusNode({
  ...
  const color = failed ? "#FF3B5C" : "#00F0FF";
  ...
  return (
    <g>
      <motion.circle
        ...
        cy={y}
        r={radius}
        fill={active ? color : "#0D0E18"}
        stroke={active ? color : "rgba(0,240,255,0.25)"}
        strokeWidth={1.5}
        animate={
          active && !revealed
      ...
      />
      <text x={x} y={y + radius + 16} textAnchor="middle" className="fill-text-muted font-mono" fontSize={9}>
        {label}
      </text>
    </g>
  );
