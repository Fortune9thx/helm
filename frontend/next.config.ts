...
const nextConfig: NextConfig = {
  ...
  async headers() {
    ...
    // (register_policy / update_policy / deactivate_policy /
    // evaluate_policy) triggered from a button click -- frame-ancestors
    // 'none' stops the page from being embedded in a hidden/disguised
    // iframe on another site for a clickjacking attempt against those
    // buttons. The rest are standard baseline hardening; Next/Vercel set
  ...
  webpack: (config) => {
    ...
    // Helm never uses x402 payments, only GenLayer contract calls.
